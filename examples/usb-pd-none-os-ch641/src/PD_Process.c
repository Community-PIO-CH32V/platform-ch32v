...
void PD_SINK_Init( )
{
    ...
    USBPD->PORT_CC1&=~CC_CVS_Mask;
#if(CC_2==CC2)
    USBPD->PORT_CC2&=~CC_CVS_Mask;
#elif(CC_2==CC3)
    USBPD->PORT_CC3&=~CC_CVS_Mask;
#endif
...
#elif(CC_2==CC3)
    USBPD->PORT_CC3 = CC_CVS_66 | CC_PD;
#endif
}
...
void PD_Init( void )
{
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB, ENABLE);               /* Open PD I/O clock, AFIO clock and PD clock */
 ...
 *
 * @return  0:No connection; 1:CC1 connection; 2:CC2 connection; 3:CC3 connection
 */
UINT8 PD_Detect( void )
{
    ...
    else                                                                /* Detect insertion */
    {
        ...
        USBPD->PORT_CC3 |= CC_CE;
        USBPD->PORT_CC3 |= CC_CVS_22;
        Delay_Us(2);
        if( USBPD->PORT_CC3 & CC_CMPO )
        {
            cmp_cc3 |= bCC_CMP_22;
        }
            ...
            if( PD_Ctl.Flag.Bit.Stop_Det_Chk == 0 )
            {
                if( (USBPD->PORT_CC1 & CC_PD) || (USBPD->PORT_CC2 & CC_PD)|| (USBPD->PORT_CC3 & CC_PD) )
                {
                    ...
                    if( status == 1 )
                    {
                        ...
                        USBPD->CONFIG |= CC_SEL_1;
                    }
                    else if(status == 2)
                    {
                        USBPD->CONFIG&=~CC_SEL_Mask;
                        USBPD->CONFIG |= CC_SEL_2;
                    }
                    else {
                        USBPD->CONFIG |= CC_SEL_2;
